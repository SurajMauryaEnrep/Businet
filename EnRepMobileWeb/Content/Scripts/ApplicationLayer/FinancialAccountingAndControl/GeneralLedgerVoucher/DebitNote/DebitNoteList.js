/************************************************
Javascript Name:Voucher(DebitNote) List
Created By:Hina Sharma
Created Date: 23-06-2022
Description: This Javascript use for the (DebitNote)Voucher Payment List Page
...
$(document).ready(function () {
    ...
    $("#DNList #datatable-buttons tbody").bind("dblclick", function (e) {
        debugger;
        try {
            var ListFilterData = $("#ListFilterData").val();
            var WF_Status = $("#WF_Status").val();
            var clickedrow = $(e.target).closest("tr");
            var VouNo = clickedrow.children("#VouNumber").text();
            var Voudt = clickedrow.children("#hdVouDate").text();
            if (VouNo != null && VouNo != "") {
                window.location.href = "/ApplicationLayer/DebitNote/EditVou/?VouNo=" + VouNo + "&Voudt=" + Voudt + "&ListFilterData=" + ListFilterData + "&WF_Status=" + WF_Status;
            }
        }
        catch (err) {
            debugger
    ...
    //    $("#datatable-buttons_wrapper .dt-buttons").append('<button class="btn btn-default buttons-csv buttons-html5 btn-sm Csv" id="" onclick="DebitNoteCSV()"  tabindex="0" aria-controls="datatable-buttons5" href="#"><span>CSV</span></button>')
    //});
});
function DebitNoteCSV() {
    ...
    window.location.href = "/ApplicationLayer/DebitNote/DebitNoteExporttoExcelDt?EtityName=" + EtityName + "&EtityType=" + EtityType + "&Fromdate=" + Fromdate + "&Todate=" + Todate + "&Status=" + Status;
}
function BindEntityListOnList() {
    ...
    $("#ddlEntityName").select2({
        ...
        //        var queryParameters = {
        //            entity_name: params.term // search term like "a" then "an"
        //            //Group: params.page
        //        };
...
function FilterDNList() {
    ...
    try {
        ...
        $.ajax({
            type: "POST",
            url: "/ApplicationLayer/DebitNote/SearchDebitNoteDetail",
            data: {      
                entity_id: EtityName,
...
function OnClickIconBtn(e) {
    ...
    var vou_no = clickedrow.find("#Dn_Vouno").text();
    var voudt = clickedrow.find("#Dn_Date").text();
