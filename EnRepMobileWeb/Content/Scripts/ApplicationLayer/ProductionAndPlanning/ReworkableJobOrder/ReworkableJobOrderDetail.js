$(document).ready(function () {
    ...
    DynamicSerchableItemDDL("", "#ddl_ItemName", "", "", "", "RwkJobOrdr");/* Added by Suraj Maurya on 28-07-2025 */
...
function OnChange_ProductName() {
    ...
    if (productid != "0" && productid != null) {
        ...
        if ((ItemID != "" && ItemID != "0") && (WarehouseID != "0" && WarehouseID != "")) {
          //  GetAvlStock(ItemID, WarehouseID)
...
function OnClickIconBtn(e, flag) {
    debugger;
    var ItmCode = "";
...
function bindworkstation(ddl_ShopfloorName) {
    $.ajax(
        {
            type: "POST",
            url: "/ApplicationLayer/ReworkableJobOrder/BindWorkStationList",
            data: {
...
function CheckHeaderValidation() {
    ...
    if (ddl_Warehouse != "0") {
        ...
        if (AvoidDot(AvailableQuantity) == false) {
            $("#AvailableQuantity").val("0.000");
            AvailableQuantity = 0;
        }
        if ((parseFloat(AvailableQuantity).toFixed(QtyDecDigit)) != "0.000" || AvailableQuantity != "") {
    ...
    if (ddl_ProductName != "0" && ddl_Warehouse != "0" && AvailableQuantity != "0.000") {
        var txtReworkQty = $("#txtReworkQuantity").val();
        if (AvoidDot(txtReworkQty) == false) {
            $("#txtReworkQuantity").val("0.000");
            txtReworkQty = 0;
        }
        var abc = parseFloat(txtReworkQty).toFixed(QtyDecDigit);
        //if ((parseFloat(txtReworkQty).toFixed(QtyDecDigit)) != "0.000" || txtReworkQty != "") {
...
function GetAvlStock(ItemID, WarehouseID, src_type, accodian_type) {
    ...
    $.ajax(
        {
            type: "POST",
            url: "/ApplicationLayer/ReworkableJobOrder/BindRewrkWHAvalStk",
            data: {
                ItemID: ItemID,
                WarehouseID: WarehouseID,
                src_type: src_type,
                accodian_type: accodian_type
            },
            dataType: "json",
            success: function (data) {
                debugger;
                if (data !== null && data !== "") {
                    ...
                    if (arr.Table.length > 0) {
                        for (var i = 0; i < arr.Table.length; i++) {
                            var AvailableQty = arr.Table[i].wh_rwk_stk_bs;
...
function BindItemRequriedMaterial() {
    var ddl_MaterialTyp = $("#ddlMaterialType option:selected").val();
    var ddl_HedrItemId = $("#ddl_ItemName option:selected").val();
    $("#ddlMaterialName").select2({
        ajax: {
            type: "POST",
            url: "/ApplicationLayer/ReworkableJobOrder/BindMaterialName",
            data: {
                ddl_MaterialTyp: ddl_MaterialTyp,
            },
            dataType: "json",
            success: function (data) {
                ...
                if (data !== null && data !== "") {
                    ...
                    if (arr.Table.length > 0) {
                        ...
                        $("#ddlMaterialName").append(`<optgroup class='def-cursor' id="Textddl2" label='${$("#ItemName").text()}' data-uom='${$("#ItemUOM").text()}'></optgroup>`);
                        $('#Textddl2').append(`<option data-uom="0" value="0">---Select---</option>`);

                        for (var i = 0; i < arr.Table.length; i++) {
                            $("#ddlMaterialName option[value=" + ddl_HedrItemId + "]").select2().hide();
                            $('#Textddl2').append(`<option data-uom="${arr.Table[i].uom_alias}" value="${arr.Table[i].item_id}">${arr.Table[i].item_name}</option>`);
                        }
...
function ddlMaterialName_onchange() {
    ...
    if (MaterialID != "0" && MaterialID != null) {
        $.ajax(
            {
                type: "POST",
                url: "/ApplicationLayer/ReworkableJobOrder/GetItemUOM",
                data: {
...
function editRow(el, e, option) {
    ...
    //$("#hdn_MaterialTypID").val(MaterialtypID);
...
function OnClickItemReworkQtyIconBtn(el, evt) {
    try {
        debugger;
        var QtyDecDigit = $("#QtyDigit").text();///Quantity
        ...
        if (RewrkJO_Status == "" || RewrkJO_Status == null || RewrkJO_Status == "D") {
            $.ajax({
                type: "Post",
                url: "/ApplicationLayer/ReworkableJobOrder/getItemReworkQuantityDetail",
                data: {
                    ItemID: ItmCode, WHID: WHID, Status: RewrkJO_Status, SelectedItemdetail: SelectedItemdetail,
                    hdnTranstyp: hdnTranstyp, hdnCommand: hdnCommand, src_type: src_type
                },
                success: function (data) {
                    debugger;
        ...
        //else if (PL_Status != "D") {
            ...
            $.ajax({
                type: "Post",
                url: "/ApplicationLayer/ReworkableJobOrder/getItemReworkQuantityDetailAfterInsert",
                data: {
                    RJO_No: RJO_No,
                    RJO_Date: RJO_Date,
 ...
 function OnClickForwardOK_Btn() {
    ...
    if (fwchkval === "Forward") {
        if (fwchkval != "" && RJONo != "" && RJODate != "" && level != "") {
            ...
            //window.location.href = "/ApplicationLayer/ReworkableJobOrder/ToRefreshByJS";
            var list = [{ RJONo: RJONo, RJODate: RJODate }];
            var FrwdDtList = JSON.stringify(list);
            window.location.href = "/ApplicationLayer/ReworkableJobOrder/ToRefreshByJS?FrwdDtList=" + FrwdDtList + "" + "&ListFilterData1=" + ListFilterData1 + "&WF_Status=" + WF_Status;
    ...
    if (fwchkval === "Approve") {
        ...
        window.location.href = "/ApplicationLayer/ReworkableJobOrder/ApproveDocByWorkFlow?AppDtList=" + AppDtList + "" + "&ListFilterData1=" + ListFilterData1 + "&WF_Status=" + WF_Status;
    }
    //if (fwchkval === "Approve") {
    //    window.location.href = "/ApplicationLayer/ReworkableJobOrder/SIListApprove?SI_No=" + RJONo + "&SI_Date=" + RJODate + "&InvType=" + InvType + "&A_Status=Approve" + "&A_Level=" + $("#hd_currlevel").val() + "&A_Remarks=" + Remarks + "&SaleVouMsg=" + SaleVouMsg;
...
function CheckedJobCompletion() {
    ...
    if ($("#JobCompletion").is(":checked")) {
        ...
        $.ajax({
            type: 'POST',
            url: '/ApplicationLayer/ReworkableJobOrder/GetDetailsOfRequiredMaterialTbl',
            data: {
                RJO_No: RJO_No,
                RJO_Date: RJO_Date,
...
function CMItemStockBatchWise(el, evt) {
    try {
        ...
        if (CMRJO_Status == "I" || CMRJO_Status=="PFC") {
            $.ajax({
                type: "Post",
                url: "/ApplicationLayer/ReworkableJobOrder/getCMItemStockBatchWiseBYShpfloor",
                data: {
                    ItemId: ItemId,
            ...
            $.ajax({
                type: "Post",
                url: "/ApplicationLayer/ReworkableJobOrder/getCMItemStockBatchWiseAfterInsert",
                data: {
                    RJO_No: RJO_No,
                    RJO_Date: RJO_Date,
                    Status: CMRJO_Status,
...
function InsertConsumeMaterialBatchDetail() {
    ...
    if (batchrowcount > 1) {
        ...
        $("#CMSaveItemBatchTbl TBODY TR").each(function () {
            var row = $(this)
            var batchList = {};
            batchList.MaterialtypID = row.find('#CM_BatchMtrlTypId').val();
            batchList.ItemId = row.find('#CM_BatchItemId').val();
            batchList.UOMId = row.find('#CM_BatchUOMId').val();
            batchList.LotNo = row.find('#CM_BatchLotNo').val();
            batchList.BatchNo = row.find('#CM_BatchBatchNo').val();
            batchList.BatchAvlStock = row.find('#CM_BatchBatchAvlStk').val();
            batchList.IssueQty = row.find('#CM_BatchConsumeQty').val();
...
function CMItemStockSerialWise(el, evt) {
    try {
        ...
        if (CMRJO_Status == "I") {
            $.ajax({
                type: "Post",
                url: "/ApplicationLayer/ReworkableJobOrder/getItemstockSerialWiseByShpFloor",
                data: {
                    ItemId: ItemId,
                    Status: CMRJO_Status,
                    ShpfloorId: ShpfloorId,
            ...
            $.ajax({
                type: "Post",
                url: "/ApplicationLayer/ReworkableJobOrder/getCMItemStockSerialWiseAfterInsert",
                data: {
                    RJO_No: RJO_No,
                    RJO_Date: RJO_Date,
                    Status: CMRJO_Status,
   ...
   var src_type= $("#hdnsrc_type").val();
    ...
    if (flag == "RJO_RewrkQuantity" || flag == "RM_RequireQty" || flag == "CM_ConsumeQty") {
        $("#hdn_Sub_ItemDetailTbl tbody tr  #ItemId[value=" + ProductId + "]").closest("tr").each(function () {
            ...
            debugger;
            if (flag == "RJO_RewrkQuantity" || flag == "CM_ConsumeQty") {
    ...
    $.ajax({
        type: "POST",
        url: "/ApplicationLayer/ReworkableJobOrder/GetSubItemDetails",
        data: {
            Item_id: ProductId,
