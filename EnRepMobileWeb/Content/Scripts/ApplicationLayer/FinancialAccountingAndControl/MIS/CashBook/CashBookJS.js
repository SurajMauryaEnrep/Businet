...
function FilterCashBookDetails() {
    debugger;
    try {
        ...
        if (Fromdate != "" && Fromdate != null) {
            $.ajax({
                type: "POST",
                url: "/ApplicationLayer/CashBook/SearchCashBookDetail",
                data: {
                    acc_id: acc_name,
                    curr_id: curr,
                    Fromdate: Fromdate,
                ...
                success: function (data) {
                    ...
                    //var opbal = $("#cb_op_bal").val();
                    //var opbaltype = $("#cb_op_bal_type").val().trim();
                    //var clbal = $("#cb_cl_bal").val();
                    //var clbaltype = $("#cb_cl_bal_type").val().trim();

                    //$("#CB_OpeningBalance").val(opbal);
                    //if (opbaltype != "0") {
                    //    $("#lbl_cb_opbaltype").text(opbaltype);
                    //    $("#Hdnlbl_cb_opbaltype").val(opbaltype);
                    //}
                    //else {
                    //    $("#lbl_cb_opbaltype").text("");
                    //}

                    //$("#CB_ClosingBalance").val(clbal);
                    //if (clbaltype != "0") {
                    //    $("#lbl_cb_clbaltype").text(clbaltype);
                    //    $("#Hdnlbl_cb_clbaltype").val(clbaltype);
                    //}
                    //else {
        ...
        debugger;
        console.log("Cash Book Error : " + err.message);

    }
}
function GeneralLedgerPrint() {
    // Commented By Suraj Maurya on 20-06-2025
    //var arr = [];

    //$("#HdnTableCashBook tbody tr").each(function () {
    //    var CurrentRow = $(this);
    //    var list = {};
...
function AccountRecevablCSV() {
    ...
    //var searchValue = $("#DataTable_CashBook_filter input[type=search]").val();
    //$("#searchValue").val(searchValue);

    var dt = $("#DataTable_CashBook").DataTable(); var order = dt.order();  // [[colIndex, 'asc/desc']]
...
function OnChnage_AccName() {
    var acc_name = $("#acc_name option:selected").val();
    var acc_nameTxt = $("#acc_name option:selected").text();
    if (acc_name == "0" || acc_name == null || acc_name == "") {
        $("#vmacc_name").text($("#valueReq").text());
        $("#vmacc_name").css("display", "block");
        $("[aria-labelledby='select2-acc_name-container']").css("border-color", "red");
    ...
    $("#hdn_Accname").val(acc_nameTxt);
    $("#hdn_AccId").val(acc_name);
}
