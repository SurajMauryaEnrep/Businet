    ...
    $('#PAlertSetupList #datatable-buttons tbody').bind("dblclick", function (e) {
        try {
            ...
            if (docId != null && eventId != "") {
                window.location.href = "/BusinessLayer/MessageSetup/EditAlertSetup/?docId=" + docId + "&events=" + eventId + "&alertType=" + alertType + "&asData=" + asData;
            }
        }
        catch (exc) {
            throw exc;
...
function BindDocumentEventList() {
    ...
    $.ajax({
        type: "POST",
        url: "/BusinessLayer/MessageSetup/BindDocumentEvents",
...
function FilterListCust() {
    ...
    $.ajax({
        type: "POST",
        url: "/BusinessLayer/MessageSetup/SearchMessageSetupDetail",
        data: {
...
function EnableDisableEmailSetup() {
    var dropdownValue = $('#ddlAlertType').val();
    if (dropdownValue == "mail") {
        $('#divEmailSetup').css('display', 'block');
        $('#divOtherSetup').css('display', 'none');
    }
    else {
        $('#divEmailSetup').css('display', 'none');
        $('#divOtherSetup').css('display', 'block');
    }
}
...
function BindAlertFields() {
    ...
    $.ajax({
        type: "POST",
        url: "/BusinessLayer/MessageSetup/GetAlertFields",
...
function ValidateEmailForm() {
    ...
    if ($.trim(alertType) == "0" || alertType == undefined) {
        $('#alertTypeMsg').html($('#valueReq').text()).css("color", "red");
