...
var Graph = Graph || (function($) {
	...
	Private.design = function(obj) {
		...
		var colors = obj.colors || ['red', 'blue', 'green', 'orange', 'yellow','purple',  'brown', 'gold', 'black', 'grey', '#dc3545', '#8bc34a', '#ce4a21', '#00bcd4', '#d4c96c', '#8ad46c', '#61172b', '#615717', '#176128', '#3fe866', '#2a3f54', '#9e9e9e', '#cddc39', '#6d7fe4','#003972'];
	...
	Graph.prototype.addLegend = function() {
		...
		if (self.multipleDataSets || self.type === 'pie') {
			for (var i = 0, len = self.numPoints; i < len; ++i) {
				...
				//TEXT
				legend += '<text style="cursor:default;display:none;"class="legend-of-' + i + '"x="' + (x + width + 5) +
...
var GraphPie = GraphPie || (function($) {
	...
	GraphPie.prototype.init = function(thing) {
		if (!this.obj.multipleDataSets) {
			...
			var HORZ=0; //x component of line
			var VERT=0; //y component of line
			var sizing = 20;
			...
			for (var i = 0; i < len; ++i) {
				...
				E.tooltips += '<text class="SVG-tooltip-title"style="display:none;font-weight:bold;font-size:15px;"id="' + self.id + '-point-' + (i) +
...
(function($) {
	$.fn.graphify = $.fn.graphify || function(options) {
		...
		this.append('<div id="' + wrapper + '"><div id="' + wrapper + '-g-area" style="text-align: center; margin-top:-40px;"></div></div>');
