...
function renderTable() {
    ...
    const end = Math.min(start + rowsPerPage, filteredDataRows.length);

    for (let k = start; k < end; k++) {
        const row = filteredDataRows[k];
        tbody.append(`
            <tr>
                <td>${row.doc_name}</td>
    ...
    $('#pageInfo').text(`Page ${filteredDataRows.length ? currentPage + 1 : 0} of ${totalPages}`);
    $('#btnPrev').prop('disabled', currentPage === 0);
    $('#btnNext').prop('disabled', currentPage >= totalPages - 1);
}

//function FilterPendingDocuments(e) { /*Commented By Nitesh 13062025 For Change Filter*/
//    debugger;
//    var listdt = e.currentTarget.value.toLocaleLowerCase();
//    var list = "";

//    $("#TblPending_Document >tbody > tr").each(function () {
//        var currentRow = $(this);
...
function FilterPendingDocuments(e) {
    ...
    filteredDataRows = dataRows.filter(row =>
        row.doc_name.toLowerCase().includes(searchTerm)
    );
    filteredDataRows.sort((a, b) => a.doc_name.localeCompare(b.doc_name));
    currentPage = 0;
    renderTable();
}
...
function handleModuleChange() {
    ...
    const misItems = document.querySelectorAll(".mis-item");

    misItems.forEach(item => {
        const itemModule = item.getAttribute("data-module");

        if (selectedModule === "0" || itemModule === selectedModule) {
            item.style.display = "";
        } else {
...
function loadDataFromServer(arr, flag) {
    if (flag === "FirstTime") {
        if (arr.Table3 && arr.Table3.length > 0) {
            dataRows = arr.Table3;
            filteredDataRows = [...dataRows].sort((a, b) => a.doc_name.localeCompare(b.doc_name));
            currentPage = 0;
            renderTable();
        } else {
            $('#TblPending_Document tbody').empty();
                ...
                if (flag == "Chart") {
                    ...
                    if (arr.Table2.length > 0) {
                        ...
                        if (currFormat == "en-in") {
                            ...
                            $("#span_AdvanceReceived").text(arr.Table6[0].curr_logo + " " + arr.Table2[0].Receivable_Adv + " L");
                            $("#span_AdvancePaid").text(arr.Table6[0].curr_logo + " " + arr.Table2[0].Payable_Adv + " L");
                            $("#span_WarehouseStock").text(arr.Table6[0].curr_logo + " " + arr.Table2[0].WhStock + " L");
                            $("#span_ShopfloorStock").text(arr.Table6[0].curr_logo + " " + arr.Table2[0].ShflStock + " L");
                            $("#span_InterBranchSales").text(arr.Table6[0].curr_logo + " " + arr.Table2[0].InterBrSale + " L");
                            $("#span_InterBranchPurchase").text(arr.Table6[0].curr_logo + " " + arr.Table2[0].InterBrPurchase + " L");
                            $("#span_NetProfit").text(arr.Table6[0].curr_logo + " " + arr.Table2[0].NetProfit + " L");
                        }
                        else {
                            ...
                            $("#span_AdvanceReceived").text(arr.Table6[0].curr_logo + " " + arr.Table2[0].Receivable_Adv + " M");
                            $("#span_AdvancePaid").text(arr.Table6[0].curr_logo + " " + arr.Table2[0].Payable_Adv + " M");
                            $("#span_WarehouseStock").text(arr.Table6[0].curr_logo + " " + arr.Table2[0].WhStock + " M");
                            $("#span_ShopfloorStock").text(arr.Table6[0].curr_logo + " " + arr.Table2[0].ShflStock + " M");
                            $("#span_InterBranchSales").text(arr.Table6[0].curr_logo + " " + arr.Table2[0].InterBrSale + " M");
                            $("#span_NetProfit").text(arr.Table6[0].curr_logo + " " + arr.Table2[0].NetProfit + " M");
                        }
                    }
                    //----------------End-----------------------
...
function Doc_RedirectToListPage(value,e,flag) {
    ...
    if (value > 0) {
        ...
        if (DocID === "105101147") {
            window.location.href = "/ApplicationLayer/SupplementaryPurchaseInvoice/GetSuppPurchaseInvoiceDashbrd/?wfStatus=" + flag;
        }
        ...
        /*Added by Surbhi on 21/07/2025 for Inter Branch Purchase Invoice(105101153)*/
        if (DocID === "105101153") {
            window.location.href = "/ApplicationLayer/InterBranchPurchase/InterBranchPurchase/?wfStatus=" + flag;
        }
        /*Added by Surbhi on 21/08/2025 for Quotation Analysis(105101125)*/
        if (DocID === "105101125") {
            window.location.href = "/ApplicationLayer/QuotationAnalysis/QuotationAnalysis/?wfStatus=" + flag;
        }
        if (DocID === "105104118") {/*Add by Hina Sharma on 12-09-2025 for payment advice*/
            window.location.href = "/ApplicationLayer/PaymentAdvice/GetPAFromDashboardList/?docid=" + DocID + "&status=" + flag;
        }
        /*Added by Surbhi on 31/10/2025 for Depreciation Process(105106105)*/
        if (DocID === "105106105") {
            window.location.href = "/ApplicationLayer/DepreciationProcess/DepreciationProcess/?wfStatus=" + flag;
        }
        /*Added by Surbhi on 31/10/2025 for Depreciation Process(105106105)*/
        if (DocID === "105101101") {
            window.location.href = "/ApplicationLayer/SupplierPriceList/SupplierPriceList/?wfStatus=" + flag;
        }
        /*Added by Surbhi on 13/01/2026 for Asset Transfer(105106115)*/
        if (DocID === "105106115") {
            window.location.href = "/ApplicationLayer/AssetTransfer/AssetTransfer/?wfStatus=" + flag;
        }
        if (DocID === "105104125") {
            window.location.href = "/ApplicationLayer/PaymentRequisition/DashboardPendingDocument/?Wfstatus=" + flag;
        }
...
function ShowMISReports(featureId) {
    ...
    if (featureId == "101175") {
        window.location.href = "/ApplicationLayer/AccountReceivable/showReportFromDashBoard/?ToDt=" + ToDt;
    }
    if (featureId == "101180") {
        window.location.href = "/ApplicationLayer/AccountPayable/showReportFromDashBoard/?ToDt=" + ToDt;
    }
    if (featureId == "101185") {
    ...
    if (featureId == "101153") {
        window.location.href = "/ApplicationLayer/PurchaseTracking/ShowReportFromDashBoard/?FromDt=" + FromDt + "&ToDt=" + ToDt;
    }
    if (featureId == "101172") {
        window.location.href = "/ApplicationLayer/SalesTracking/ShowReportFromDashBoard/?FromDt=" + FromDt + "&ToDt=" + ToDt;
    }
    if (featureId == "101222") {
