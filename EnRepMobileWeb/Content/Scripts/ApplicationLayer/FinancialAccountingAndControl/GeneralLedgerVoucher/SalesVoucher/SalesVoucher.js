$(document).ready(function () {
    ...
    SVNo = $("#SVNo").val();
    $("#hdDoc_No").val(SVNo);
    $('#SVdetailTbl').on('click', '.deleteIcon', function () {
...
function updateItemSerialNumber() {
    debugger;
    var SerialNo = 0;
    $("#SVdetailTbl >tbody >tr").each(function (i, row) {
        debugger;
        var currentRow = $(this);
        SerialNo = SerialNo + 1;
...
function BindDDLAccountList() {
    ...
    Cmn_BindAccountList("#GLAccount_", "1", "#SVdetailTbl", "#SNohf", "BindData", "105104115145");
...
function BindData() {
    ...
    if (AccountListData != null) {
        if (AccountListData.length > 0) {
            $("#SVdetailTbl >tbody >tr").each(function () {
                var currentRow = $(this);
                var rowid = currentRow.find("#SNohf").val();
                debugger;
                rowid = parseFloat(rowid) + 1;
                if (rowid > $("#SVdetailTbl >tbody >tr").length) {
                    return false;
                }
                ...
                $("#GLAccount_" + rowid).select2({
                    ...
                    templateResult: function (data) {
                        ...
                        if (check(data, selected, "#SVdetailTbl", "#SNohf", "#GLAccount_") == true) {
...
function BindCustAccList() {
    debugger;
    $("#ddlCustAccName").select2({
        ajax: {
            url: $("#hdCustList").val(),
...
function OnChangeCustCoa() {
    debugger;
    var RateDecDigit = $("#RateDigit").text();
    var ExchDecDigit = $("#ExchDigit").text();
    var coa_id = $("#ddlCustAccName").val();
    if (coa_id == "" || coa_id == "0" || coa_id == null) {
        $("[aria-labelledby='select2-ddlCustAccName-container']").css("border-color", "red");
        $("#vmCustAccName").text($("#valueReq").text());
        $("#vmCustAccName").css("display", "block");
        $("#ddlCustAccName").css("border-color", "red");
        ...
        $("#vmCustAccName").css("display", "none");
        //$("#ddlCustAccName").css("border-color", "#ced4da");
        $("[aria-labelledby='select2-ddlCustAccName-container']").css("border-color", "#ced4da");
    ...
    $.ajax(
        {
            type: "POST",
            url: "/ApplicationLayer/SalesVoucher/GetAccCurr",
            data: {
                acc_id: coa_id,
            },
            success: function (data) {
                ...
                if (data !== null && data !== "") {
                    ...
                    if (arr.Table.length > 0) {
                        ...
                        $("#hdCustAccId").val(arr.Table3[0].acc_id);
                    }
...
function OnChangeConvRate(e) {
    ...
    if (ConvRate == "" || ConvRate == null || parseFloat(ConvRate) == parseFloat("0")) {
        ...
        var ConvRate2 = toDecimal(cmn_ReplaceCommas(ConvRate), ExchDecDigit);
...
function AddCustDetail() {
    ...
    var CustAccID = $("#ddlCustAccName").val();
    var ConvRate = $("#ConvRate").val();
    var VouDate = $("#SVDate").val();/*add by Hina Sharma on 27-08-2025*/
    if (CustAccID != "0" && CustAccID != "" && CustAccID != null && ConvRate != "" && ConvRate != null) {
        $("#Plusbankdt").css('display', 'none');
        $("#BtnAddGL").css('display', 'block');
        debugger;
        $("#ddlCustAccName").prop("disabled", true);
        $("#SVHeaderNarration").attr("disabled", true);
        var CC_Acc_id = CustAccID.substring(0, 1);
        ...
        $.ajax(
            {
                type: "Post",
                url: "/ApplicationLayer/SalesVoucher/GetCustAccIDDetail",
                data: {
                    CustAccID: CustAccID,
                    VouDate: VouDate
                },
                            ...
                            for (var i = 0; i < arr.Table.length; i++) {
                                ...
                                $("#SVdetailTbl >tbody >tr").each(function () {
                                    ...
                                    if (a < arr.Table.length) {
                                        ...
                                        currentRow.find("#RowWiseAccBal").val(arr.Table1[a].Avl_bal);/*add AccBal by Hina On 28-08-2025 */
                                        ...
                                        currentRow.find("#Narration").val($("#SVHeaderNarration").val());
                                    }
                                    a = a + 1;
                                });
                            }
...
function AddNewRow() {
    var rowIdx = 0;
    debugger;
    var rowCount = $('#SVdetailTbl >tbody >tr').length + 1
    var RowNo = 0;
    $("#SVdetailTbl >tbody >tr").each(function (i, row) {
        debugger;
        var currentRow = $(this);
    ...
    $('#SVdetailTbl tbody').append(`<tr id="${++rowIdx}">
...
<td>
<div class="lpo_form">
<textarea id="Narration" class="form-control remarksmessage" maxlength="200" onkeyup="OnchangeNarr(event)" onchange="OnchangeNarr(event)" autocomplete="off" type="text" onmouseover="OnMouseOver(this)" name="Narration" placeholder="${$("#span_Narration").text()}">${$("#SVHeaderNarration").val()}</textarea>
...
<td style="display: none;"><input type="hidden" id="SNohf" value="${RowNo}" /></td>
    ...
    /*commenetd and modify by Hina on 28-08-2025 above for show account balance*/
...
function BindAcountList(ID) {
    ...
    Cmn_BindAccountList("#GLAccount_", ID, "#SVdetailTbl", "#SNohf", "", "105104115145");
...
function BindJornlVoucAccountList(e) {
    ...
    /*-----------Code Start Add by Hina Sharma on 28-08-2025 to get account Balance Account wise-------- */
    var VouDate = $("#SVDate").val();
    CMN_BindAccountBalance(clickedrow, Acc_ID, VouDate);
    /*-----------Code End Add by Hina Sharma on 28-08-2025 to get account Balance Account wise-------- */
}
...
function OnChangeDebitAmount(e) {
    ...
    if (AvoidDot(debitAmt) == false) {
        ...
        //currentrow.find("#DebitAmountInSpecific").val(parseFloat(debitAmt3).toFixed(ValDecDigit))
    }
...
function CalculateTotalDrCrValue() {
    ...
    $("#SVdetailTbl tbody tr").each(function () {
        var currentrow = $(this);
        debugger;
        /*Commented by HIna on 30-10-2023 to changes for Base Amount instead of specific amount*/
...
function InsertSVDetail() {
    ...
    FinalAccountDetail = InsertSVAccountDetails();
    var AccountDt = JSON.stringify(FinalAccountDetail);
    $('#hdAccountDetailList').val(AccountDt);
    CalculateTotalDrCrValue();


    if (Cmn_CC_DtlSaveButtonClick("SVdetailTbl", "DebitAmountInSpecific", "CreditAmountInSpecific","hdSVStatus") == false) {
        return false;
    }
    ...
    /*----- Attatchment End--------*/
    let VouAmnt = $("#hdnVouAmnt").val();
    $("#hdnVouAmnt").val(cmn_ReplaceCommas(VouAmnt));
...
function InsertSVAccountDetails() {
    debugger;
    ...
    $("#SVdetailTbl TBODY TR").each(function () {
        ...
        AccountList.cust_acc_id = $("#hdCustAccId").val();
...
function AccountValidation() {
    ...
    var DocId = $("#DocumentMenuId").val();
    var ErrorFlag = "N";
    if ($("#SVdetailTbl tbody tr").length > 0) {
...
function ForwardBtnClick() {
    debugger;
    //var SVStatus = "";
    //SVStatus = $('#hdSVStatus').val().trim();
    //if (SVStatus === "D" || SVStatus === "F") {
...
async function OnClickForwardOK_Btn() {
    ...
    if (fwchkval === "Forward") {
        if (fwchkval != "" && VouNo != "" && VouDate != "" && level != "") {
            ...
            window.location.href = "/ApplicationLayer/SalesVoucher/ToRefreshByJS?ListFilterData1=" + ListFilterData1 + "&ModelData=" + ModelData;
        }
    }
    ...
    if (fwchkval === "Approve") {
        window.location.href = "/ApplicationLayer/SalesVoucher/SalesVoucherApprove?VouNo=" + VouNo + "&VouDate=" + VouDate + "&A_Status=Approve" + "&A_Level=" + $("#hd_currlevel").val() + "&A_Remarks=" + Remarks + "&ListFilterData1=" + ListFilterData1 + "&WF_Status1=" + WF_Status1;
...
function GetPdfFilePathToSendonEmailAlert(poNo, poDate, fileName) {
    ...
    $.ajax({
        type: "POST",
        url: "/ApplicationLayer/SalesVoucher/SavePdfDocToSendOnEmailAlert",
...
function CheckedCancelled() {
    ...
    if ($("#Cancelled").is(":checked")) {
        ...
        $("#btn_save").attr("onclick", "return InsertSVDetail()");
...
function VoucherDateValidation() {
    ...
    Cmn_VoucherDateValidation("#SVDate");
