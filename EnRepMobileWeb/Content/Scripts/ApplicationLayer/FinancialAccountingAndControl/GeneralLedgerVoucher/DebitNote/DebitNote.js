...
$(document).ready(function () {
    ...
    BindEntityList();
    BindDDLAccountList();
    DNNo = $("#DebitNoteNo").val();
    $("#hdDoc_No").val(DNNo);
    $("#Entity_Type").on("change", function () {
        BindEntityList();
        onchangeEntityType("EnableBillD");
    ...
    $('#DNdetailTbl').on('click', '.deleteIcon', function () {
...
function hdnAccGrpval() {
    debugger;
    $("#DNdetailTbl > tbody > tr").each(function () {
        debugger;
        var CurrRow = $(this);
...
function BindEntityList() {
    ...
    var flag = " ----Select----";
    if (Entitytype == "1") {
        $("#DNdetailTbl>tbody >tr >#td_BillAdjustment").css("display", "none")
        $("#DNdetailTbl>thead >tr >#th_BillAdjustment").css("display", "none")
    }
    else {
        $("#DNdetailTbl>tbody >tr >#td_BillAdjustment").css("display", "")
        $("#DNdetailTbl>thead >tr >#th_BillAdjustment").css("display", "")
    }
...
function OnChangeEntityCoa() {
    ...
    $.ajax(
        {
            type: "POST",
            url: "/ApplicationLayer/DebitNote/GetAccCurr",
            data: {
                acc_id: coa_id,
            },
            success: function (data) {
                ...
                if (data !== null && data !== "") {
                    ...
                    if (arr.Table.length > 0) {
                        ...
                        $("#ConvRate").val(parseFloat(toDecimal(arr.Table[0].conv_rate, ExchDecDigit)));
                        $("#hdconv").val(parseFloat(toDecimal(arr.Table[0].conv_rate, ExchDecDigit)));
                        ...
                        $("#ConvRate").val(parseFloat(toDecimal(arr.Table1[0].conv_rate, ExchDecDigit)));
                        $("#hdconv").val(parseFloat(toDecimal(arr.Table1[0].conv_rate, ExchDecDigit)));
...
function AddEntityDetail() {
    ...
    var VouDate = $("#DNDate").val();/*add by Hina Sharma on 27-08-2025*/
    ...
    if ($('#ddlEntityName').val() != "0" && $('#ddlEntityName').val() != "" && ConvRate != "" && ConvRate != null) {
        ...
        $("#DnHeaderNarration").attr("disabled", true);
        ...
        $.ajax(
            {
                type: "Post",
                url: "/ApplicationLayer/DebitNote/GetEntityIDDetail",
                data: {
                    EntityID: EntityID,
                    VouDate: VouDate
                },
                                ...
                                $("#DNdetailTbl >tbody >tr").each(function () {
                                    ...
                                    if (a < arr.Table.length) {
                                        ...
                                        if (arr.Table[i].cr_amt_bs == "0") {
                                            ...
                                            // currentRow.find("#CreditAmountInSpecific").val(parseFloat(arr.Table[i].cr_amt_bs).toFixed(ValDecDigit));
                                            currentRow.find("#CreditAmountInSpecific").val(toDecimal(arr.Table[i].cr_amt_bs, ValDecDigit));
                                        }
                                        if (arr.Table[i].cr_amt_bs == "0") {
                                        ...
                                        currentRow.find("#Narration").val($("#DnHeaderNarration").val());
                                        if (arr.Table[a].acc_type.toString() == "2") {
                                            currentRow.find("#BillAdjustment").attr("disabled", false);
                                            currentRow.find("#BillAdjustment").attr("onclick", "OnClickBillAdjIconBtn(event)");
                                        } else {
...
function VoucherDateValidation() {
    ...
    if (VouDate == "") {/*code start Add by Hina on 12-11-2024 to validate voucher date */
        ...
        $("#DNDate").css("border-color", "Red");
        return false
    }
    else {
        $("#SpanVouDateErrorMsg").css("display", "none");
        $("#DNDate").css("border-color", "#ced4da");/*code End Add by Hina on 12-11-2024 to validate voucher date */
        Cmn_VoucherDateValidation("#DNDate");
    }
...
function OnChangeCreditAmount(e) {
    ...
    if (AvoidDot(creditAmt) == false) {
        ...
        // var creditAmt2 = parseFloat(creditAmt).toFixed(ValDecDigit); /**Commented By Nitesh 16-02-2024 Add Decimal Replace in Fixed**/
    ...
    if (Rowno == 1) {
        ...
        if (creditAmt != "") {
            ...
            currentrow.find("#DebitAmountInSpecific").attr("disabled", false);
            currentrow.find("#DebitAmountInSpecific").removeAttr("readonly");
            //currentrow.find("#DebitAmountInSpecific").css("border-color", "#ced4da");
            //currentrow.find("#Dbt_Amnt_Error").text("");
            //currentrow.find("#Dbt_Amnt_Error").css("display", "none");
        }
    ...
    if ((creditAmt !== 0 || creditAmt !== null || creditAmt !== "") && (ConvRate !== 0 || ConvRate !== null || ConvRate !== "")) {
        debugger;
        var FAmt = creditAmt * ConvRate;
        //var FinVal1 = parseFloat(FAmt).toFixed(ValDecDigit); /**Commented By Nitesh 16-02-2024 Add Decimal Replace in Fixed**/
        var FinVal1 = toDecimal(cmn_ReplaceCommas(FAmt), ValDecDigit);
        var FinVal = cmn_addCommas(FinVal1);
        currentrow.find("#CreditAmountInBase").val(FinVal);
    }
...
function HeaderValidation() {
    debugger;
    var ErrorFlag = "N";
    var voudate = $("#DNDate").val();/*Add by Hina on 12-11-2024 to validate voucher date on save */
    if (voudate == "") {
        $('#SpanVouDateErrorMsg').text($("#valueReq").text());
        $("#SpanVouDateErrorMsg").css("display", "block");
        $("#DNDate").css("border-color", "Red");
        ...
        $("#SpanVouDateErrorMsg").css("display", "none");
        $("#DNDate").css("border-color", "#ced4da");
...
function AccountValidation() {
    ...
    var EntityType = $("#Entity_Type").val();
    var ErrorFlag = "N";
    if ($("#DNdetailTbl tbody tr").length > 0) {
        if (DocId == '105104115130') {
            $("#DNdetailTbl tbody tr").each(function () {
                debugger
                var currentrow = $(this);
...
function AddNewRow() {
    ...
    debugger;
    var DocId = $("#DocumentMenuId").val();
    var rowCount = $('#DNdetailTbl >tbody >tr').length + 1
    var RowNo = 0;

    $("#DNdetailTbl >tbody >tr").each(function (i, row) {
        debugger;
        var currentRow = $(this);
    ...
    if (DocId == '105104115130') {
        $('#DNdetailTbl tbody').append(`<tr id="${++rowIdx}">
...
<td>
<div class="lpo_form">
    <textarea id="Narration" class="form-control remarksmessage" maxlength="200" onkeyup="OnchangeNarr(event)" onchange="OnchangeNarr(event)" autocomplete="off" type="text" onmouseover="OnMouseOver(this)" name="Narration" placeholder="${$("#span_Narration").text()}">${$("#DnHeaderNarration").val()}</textarea>
       <span id="Narr_Error" class="error-message is-visible"></span>
 </div>
</td>
  <td class="center" id="td_BillAdjustment">
          <button type="button" disabled id="BillAdjustment" onclick="OnClickBillAdjIconBtn(event)" class="calculator subItmImg" data-toggle="modal" data-target="#BillAdjustment" data-backdrop="static" data-keyboard="false"><i class="fa fa-eye" aria-hidden="true" title="${$("#span_BillAdjustment").text()}"></i></button>
 </td>
<td class="center">
      <button type="button" disabled id="BtnCostCenterDetail" onclick="Onclick_CCbtn('type',event)" class="calculator subItmImg" data-toggle="modal" data-target="#CostCenterDetail" data-backdrop="static" data-keyboard="false"><i class="fa fa-eye" aria-hidden="true" title="${$("#span_CostCenterDetail").text()}"></i></button>
 </td>
<td style="display: none;"><input type="hidden" id="SNohf" value="${RowNo}" /></td>
</tr>`);
        /*COMMENTED BY HINA on 27-08-2025 to verify above code  */
...
<div class="lpo_form">
  ...
  <td class="center" id="td_BillAdjustment">
          <button type="button" disabled id="BillAdjustment" onclick="onclick="OnClickBillAdjIconBtn(event)" class="calculator subItmImg" data-toggle="modal" data-target="#BillAdjustment" data-backdrop="static" data-keyboard="false"><i class="fa fa-eye" aria-hidden="true" title="${$("#span_BillAdjustment").text()}"></i></button>
 </td>
<td class="center">
      <button type="button" disabled id="BtnCostCenterDetail" onclick="Onclick_CCbtn('type',event)" class="calculator subItmImg" data-toggle="modal" data-target="#CostCenterDetail" data-backdrop="static" data-keyboard="false"><i class="fa fa-eye" aria-hidden="true" title="${$("#span_CostCenterDetail").text()}"></i></button>
 </td>
...
function BindAcountList(ID) {
    ...
    Cmn_BindAccountList("#GLAccount_", ID, "#DNdetailTbl", "#SNohf", "", "105104115130");
...
function BindDDLAccountList() {
    ...
    Cmn_BindAccountList("#GLAccount_", "1", "#DNdetailTbl", "#SNohf", "BindData", "105104115130");
    ...
    if (AccountListData != null) {
        if (AccountListData.length > 0) {
            $("#DNdetailTbl >tbody >tr").each(function () {
                ...
                if (rowid > $("#DNdetailTbl >tbody >tr").length) {
                    return false;
                }
                ...
                $("#GLAccount_" + rowid).select2({
                    ...
                    templateResult: function (data) {
                        ...
                        if (check(data, selected, "#DNdetailTbl", "#SNohf", "#GLAccount_") == true) {
...
function InsertDNDetail() {
    ...
    FinalAccountDetail = InsertDNAccountDetails();
    ...
    if (Cmn_CC_DtlSaveButtonClick("DNdetailTbl", "DebitAmountInSpecific", "CreditAmountInSpecific","hdDNStatus") == false) {
        return false;
    }
...
function InsertDNAccountDetails() {
    debugger;
...
function CalculateTotalDrCrValue() {
    ...
    // var creditAmount2 = parseFloat(creditAmount).toFixed(ValDigit);/***Commented By NItesh 16-02-2024 Repalce ToFixed to Deciamal Function***/
...
function CheckedCancelled() {
    if ($("#Cancelled").is(":checked")) {
        ...
        $("#btn_save").attr("onclick", "return InsertDNDetail()");
...
async function OnClickForwardOK_Btn() {
    ...
    if (fwchkval != "Approve") {
        var pdfAlertEmailFilePath = "DebitNote_" + GetDatetimeYYYYMMDDHHMMSS() + ".pdf";
        var pdfAlertEmailFilePath1 = await Cmn_GetPdfFilePathToSendonEmailAlert(VouNo, VouDate, pdfAlertEmailFilePath, docid, fwchkval, "/ApplicationLayer/DebitNote/SavePdfDocToSendOnEmailAlert");
    ...
    if (fwchkval === "Forward") {
        if (fwchkval != "" && VouNo != "" && VouDate != "" && level != "") {
            ...
            window.location.href = "/ApplicationLayer/DebitNote/ToRefreshByJS?ListFilterData1=" + ListFilterData1 + "&ModelData=" + ModelData + "&Mailerror=" + mailerror;
        }
    }
    if (fwchkval === "Approve") {
        window.location.href = "/ApplicationLayer/DebitNote/DebitNoteApprove?VouNo=" + VouNo + "&VouDate=" + VouDate + "&A_Status=Approve" + "&A_Level=" + $("#hd_currlevel").val() + "&A_Remarks=" + Remarks + "&ListFilterData1=" + ListFilterData1 + "&WF_Status1=" + WF_Status1;
...
//    $.ajax({
//        type: "POST",
//        url: "/ApplicationLayer/DebitNote/SavePdfDocToSendOnEmailAlert",
...
function Onclick_CCbtn(flag, e) {
    ...
    $.ajax({
        type: "POST",
        url: "/ApplicationLayer/DebitNote/GetCstCntrtype",
        data: {
            Flag: flag,
...
function OnClickBillAdjIconBtn(e) {
    debugger;
    //BindDate();/*Commented and modify by Hina on 12-11-2024*/
    var clickedrow = $(e.target).closest("tr");
    var voudate = $("#DNDate").val();/*code start Add by Hina on 11-11-2024 to validate voudate */
    if (voudate == "") {
        $('#SpanVouDateErrorMsg').text($("#valueReq").text());
        $("#SpanVouDateErrorMsg").css("display", "block");
        $("#DNDate").css("border-color", "Red");
...
function BillWiseDetail(AccID) {
    ...
    var ValDecDigit = $("#ValDigit").text();///Amount
    docid = $("#DocumentMenuId").val();
    var DocumentNumber = $("#DebitNoteNo").val();
    var Status = $("#hdDNStatus").val();
...
function OnClickSaveAndExitBillDt() {
    ...
    if (CheckBillDetailValidation() === true) {
        ...
        if (parseFloat(cmn_ReplaceCommas(IconPayAmt)) >= parseFloat(cmn_ReplaceCommas(TotalPaidAmt))) {
            ...
            var row = $("#DNdetailTbl > tbody >tr #hfAccID[value=" + AccID + "]").closest('tr');
