...
namespace EnRepMobileWeb.Areas.ApplicationLayer.Controllers.Procurement.InterBranchPurchase
{
    public class InterBranchPurchaseController : Controller
    {
        ...
        Common_IServices _Common_IServices;
        InterBranchPurchase_IService _IBP_ISERVICE;
        CommonController cmn = new CommonController();
        public InterBranchPurchaseController(Common_IServices _Common_IServices, InterBranchPurchase_IService _IBP_ISERVICE, TransporterSetup_ISERVICES trptIService)
        {
            this._Common_IServices = _Common_IServices;
            this._IBP_ISERVICE = _IBP_ISERVICE;
            this._trptIService = trptIService;
        }
        // GET: ApplicationLayer/InterBranchPurchase
        public ActionResult InterBranchPurchase(string wfStatus)
        {
            try
            {
                ...
                CommonPageDetails();
                IBPListModel _IBPListModel = new IBPListModel();
                if (wfStatus != null)
                {
                    _IBPListModel.wfstatus = wfStatus;
                    _IBPListModel.ListFilterData = "0,0,0,0" + "," + wfStatus;
                }
                ...
                if (_IBPListModel.IBP_FromDate == null)
                {
                    _IBPListModel.FromDate = startDate;
                    _IBPListModel.ToDate = todate;
                }
                else
                {
                    _IBPListModel.FromDate = _IBPListModel.IBP_FromDate;
                }
                ...
                _IBPListModel.StatusList = statusLists;
                if (TempData["UrlData"] != null)
                {
                    if (TempData["UrlData"].ToString() != "" && TempData["UrlData"] != null)
                    {
                        UrlData urlData = TempData["UrlData"] as UrlData;
                        if (urlData!=null && urlData.ListFilterData1 != null && urlData.ListFilterData1 != "undefined")
                        {
                            var arr = urlData.ListFilterData1.Split(',');
                            _IBPListModel.SupplierID = arr[0];
                            _IBPListModel.FromDate = arr[1];
                            _IBPListModel.ToDate = arr[2];
                            _IBPListModel.Status = arr[3];
                            _IBPListModel.wfstatus = arr[4];
                            if (wfStatus == null)
                            {
                                wfStatus = arr[3];
                            }
                            _IBPListModel.ListFilterData = _IBPListModel.SupplierID + "," + _IBPListModel.FromDate + "," + _IBPListModel.ToDate + "," + _IBPListModel.Status + "," + wfStatus;
                        }
                    }
                }

                GetAllData(_IBPListModel);
                _IBPListModel.Title = title;
                _IBPListModel.DocumentMenuId = DocumentMenuId;
                return View("~/Areas/ApplicationLayer/Views/Procurement/InterBranchPurchase/InterBranchPurchaseList.cshtml", _IBPListModel);
            }
            catch (Exception ex)
            {
        ...
        public ActionResult IBPListSearch(string SuppId, string Fromdate, string Todate, string Status, string wfStatus)
        {
            try
            {
                CommonPageDetails();
                DataSet ds = new DataSet();
                ds = _IBP_ISERVICE.GetAllData(CompID, "", BrchID, UserID, SuppId, Fromdate, Todate, Status, "105101153", wfStatus);
                ...
                return PartialView("~/Areas/ApplicationLayer/Views/Shared/PartialInterBranchPurchaseList.cshtml");
            }
            catch (Exception ex)
            {
                string path = Server.MapPath("~");
        ...
        private void GetAllData(IBPListModel _IBPListModel)
        {
            try
            {
                string SupplierName = string.Empty;
                CommonPageDetails();
                if (string.IsNullOrEmpty(_IBPListModel.SupplierName))
                {
                    SupplierName = "0";
                }
                else
                {
                    SupplierName = _IBPListModel.SupplierName;
                }
                DataSet CustList = _IBP_ISERVICE.GetAllData(CompID, SupplierName, BrchID, UserID, _IBPListModel.SupplierID, _IBPListModel.FromDate, _IBPListModel.IBP_ToDate, _IBPListModel.Status, "105101153", _IBPListModel.wfstatus);

                List<SupplierName> _CustList = new List<SupplierName>();
                foreach (DataRow data in CustList.Tables[0].Rows)
                {
                    SupplierName _CustDetail = new SupplierName();
                    _CustDetail.supp_id = data["supp_id"].ToString();
                    _CustDetail.supp_name = data["supp_name"].ToString();
                    _CustList.Add(_CustDetail);
                }
                _CustList.Insert(0, new SupplierName() { supp_id = "0", supp_name = "All" });
                _IBPListModel.SupplierNameList = _CustList;
        ...
        //}
        //public ActionResult InterBranchPurchaseDetail()
        //{
        //    try
        //    {
        //        ViewBag.MenuPageName = getDocumentName();
        ...
        public ActionResult AddInterBranchPurchaseDetail(string DocNo, string DocDate, string ListFilterData)
        {
            try
            {
                ...
                if (DocNo == null)
                {
                    if (commCont.CheckFinancialYear(CompID, BrchID) == "Not Exist")
                    {
                        ...
                        return RedirectToAction("InterBranchPurchase", "InterBranchPurchase", urlData);
                    }
                }
                ...
                return RedirectToAction("InterBranchPurchaseDetail", "InterBranchPurchase", urlData);
            }
            catch (Exception ex)
            {
                string path = Server.MapPath("~");
        ...
        public DataTable GetWarehouseList()
        {
            ...
            {
                GetCompDeatil();
                DataSet result = _IBP_ISERVICE.GetWarehouseList(CompID, BrchID);
                dt = result.Tables[0];
            }
            catch (Exception Ex)
            {
        ...
        public DataTable GetBranchList()
        {
            ...
            {
                GetCompDeatil();
                DataSet result = _IBP_ISERVICE.GetBranchList(CompID, BrchID);
                dt = result.Tables[0];
            }
            catch (Exception Ex)
            {
        ...
        public ActionResult GetBillList(InterBranchPurchase_Model _IBPListModel, string BranchId)
        {
            //DataTable dt = new DataTable();
            try
            {
                Dictionary<string, string> BNtList = new Dictionary<string, string>();
                GetCompDeatil();
                // BNtList = _IBP_ISERVICE.GetBillNoList(CompID, BrchID,null);
                BNtList = _IBP_ISERVICE.GetBillNoList(CompID, BranchId, null);
                //dt = result.Tables[0];

                List<ibp_BillNo> _BNList = new List<ibp_BillNo>();
                foreach (var data in BNtList)
                {
                    ibp_BillNo _BNDetail = new ibp_BillNo();
                    _BNDetail.ibp_billno_id = data.Key;
                    _BNDetail.ibp_billno = data.Value;
                    _BNList.Add(_BNDetail);
                }
                _IBPListModel.ibp_BillNoList = _BNList;
                return Json(BNtList.Select(c => new { Name = c.Key, ID = c.Value }).ToList(), JsonRequestBehavior.AllowGet);
            }
            catch (Exception Ex)
            {
                string path = Server.MapPath("~");
        ...
        public JsonResult GetInterBranchSaleInvoiceDetail(string invno, string invdt, string InterbrId,string supp_id)
        {
            DataTable dt = new DataTable();
            ...
            {
                GetCompDeatil();
                DataSet Details = _IBP_ISERVICE.GetInterBranchSaleInvoiceDetail(CompID, InterbrId, invno, invdt, BrchID, supp_id);
                //dt = result.Tables[0];
                ViewBag.ItemStockBatchWise = Details.Tables[4];
                ViewBag.ItemStockSerialWise = Details.Tables[5];
        ...
        public ActionResult GetAutoCompleteSearchSuppList(InterBranchPurchase_Model _model)
        {
            string SupplierName = string.Empty;
            ...
            {
                GetCompDeatil();
                if (string.IsNullOrEmpty(_model.IBP_SuppName))
                {
                    SupplierName = "0";
                }
                else
                {
                    SupplierName = _model.IBP_SuppName;
                }
                SuppList = _IBP_ISERVICE.GetSupplierList(CompID, SupplierName, BrchID);
                List<SupplierName> _SuppmrList = new List<SupplierName>();
                foreach (var data in SuppList)
                {
                    ...
                    _SuppmrList.Add(_SuppDetail);
                }
                _model.SupplierNameList = _SuppmrList;
            }
            catch (Exception Ex)
            {
                string path = Server.MapPath("~");
        ...
        public DataSet GetIBSEdit(string Inv_No, string Inv_Date)
        {
            try
            {
                string Voutype = "SV";
                GetCompDeatil();
                DataSet result = _IBP_ISERVICE.GetIBPDetail(CompID, BrchID, Voutype, Inv_No, Inv_Date, UserID, DocumentMenuId);
                return result;
            }
            catch (Exception Ex)
            {
        ...
        public ActionResult InterBranchPurchaseDetail(UrlData urlData)
        {
            try
            {
                CommonPageDetails();
                ViewBag.DocID = "105101153";
                InterBranchPurchase_Model model = new InterBranchPurchase_Model();
                ...
                List<ibp_Branch> BranchLists = new List<ibp_Branch>();
                dt1 = GetBranchList();
                foreach (DataRow dr in dt1.Rows)
                {
                    ibp_Branch BranchList = new ibp_Branch();
                    BranchList.ibp_brch_id = dr["brchId"].ToString();
                    BranchList.ibp_brch_name = dr["BrchName"].ToString();
                    BranchLists.Add(BranchList);
                }
                BranchLists.Insert(0, new ibp_Branch() { ibp_brch_id = "0", ibp_brch_name = "---Select---" });
                model.ibp_BranchList = BranchLists;
                ...
                if (model.Inv_no != null && model.Inv_dt != null)
                {
                    ...
                    model.interbrnchid = ds.Tables[0].Rows[0]["inter_br_id"].ToString();
                    model.IBP_SuppID = ds.Tables[0].Rows[0]["supp_id"].ToString();
                    ...
                    model.bill_date = ds.Tables[0].Rows[0]["bill_dt"].ToString();
                    ...
                    model.bill_add_id = ds.Tables[0].Rows[0]["address_id"].ToString();
                    ...
                    model.EInvoive = ds.Tables[0].Rows[0]["einv_no"].ToString().Trim();
                    model.EWBNNumber = ds.Tables[0].Rows[0]["ewb_no"].ToString().Trim();
                    model.remarks = ds.Tables[0].Rows[0]["remarks"].ToString().Trim();
                    if (ds.Tables[0].Rows[0]["state_code"] == ds.Tables[0].Rows[0]["state_code"])
                    {
                ...
                suppLists.Add(new SupplierName { supp_id = model.IBP_SuppID, supp_name = model.IBP_SuppID });
                model.SupplierNameList = suppLists;

                List<ibp_BillNo> BillNoLists = new List<ibp_BillNo>();
                BillNoLists.Add(new ibp_BillNo { ibp_billno_id = model.bill_no, ibp_billno = model.bill_no });
                model.ibp_BillNoList = BillNoLists;
                GetAutoCompleteSearchSuppList(model);
                model.Title = title;
                model._ModelCommand = model.Command;
                model.DocumentMenuId = DocumentMenuId;
                ViewBag.DocumentMenuId = DocumentMenuId;
                return View("~/Areas/ApplicationLayer/Views/Procurement/InterBranchPurchase/InterBranchPurchaseDetail.cshtml", model);
            }
            catch (Exception ex)
            {
                string path = Server.MapPath("~");
        ...
        public ActionResult ActionIBPDeatils(InterBranchPurchase_Model _model, string Command)
        {
            try
            {
                var commCont = new CommonController(_Common_IServices);
                ...
                switch (Command)
                {
                    case "AddNew":
                        ...
                        if (commCont.CheckFinancialYear(CompID, BrchID) == "Not Exist")
                        {
                            ...
                            if (!string.IsNullOrEmpty(_model.Inv_no))
                            {
                                ...
                                return RedirectToAction("InterBranchPurchaseDetail", _model);
                            }
                            else
                            {
                        ...
                        return RedirectToAction("InterBranchPurchaseDetail", urlData);
                    case "Edit":
                        if (Session["CompId"] != null)
                            CompID = Session["CompId"].ToString();
                        if (Session["BranchId"] != null)
                    ...
                    case "Approve":
                        ...
                        ApproveIBPDetails(_model, _model.Inv_no, _model.Inv_dt, "", "", "", _model.Nurration, "", "", "", _model.BP_Nurration, _model.DN_Nurration);
                    ...
                    case "BacktoList":
                        ...
                        return RedirectToAction("InterBranchPurchase");
                    case "Print":
        ...
        private void DeleteIBPDetail(InterBranchPurchase_Model _model)
        {
            try
            {
                if (Session["CompId"] != null)
                ...
                string Result = _IBP_ISERVICE.DeleteIBPDetail(CompID, BrchID, _model.Inv_no, _model.Inv_dt);
        ...
        public JsonResult CheckRoundOffAccount()
        {
            try
            {
                ...
                DataTable result = _IBP_ISERVICE.CheckRoundOffAcc(Comp_ID, Br_ID);
            ...
            try
            {
                ...
                if (BatchItemDeatilData != null)
                {
                    ...
                    for (int i = 0; i < obj.Count; i++)
                    {
                        ...
                        if (obj[i]["BatchExDate"].ToString() == "" || obj[i]["BatchExDate"].ToString() == null || obj[i]["BatchExDate"].ToString() == "null" || obj[i]["BatchExDate"].ToString() == "undefined")
                        {
        ...
        private DataTable ToDtblHeaderDetail(InterBranchPurchase_Model _model)
        {
            try
            {
                ...
                dtheader.Columns.Add("address", typeof(string));
                dtheader.Columns.Add("inter_br_id", typeof(string));
                ...
                dtheader.Columns.Add("oc_amt_with_tp", typeof(string));
                ...
                dtheader.Columns.Add("app_id", typeof(string));
                dtheader.Columns.Add("app_dt", typeof(string));
                dtheader.Columns.Add("mod_id", typeof(string));
                dtheader.Columns.Add("mod_dt", typeof(string));
                ...
                dtrowHeader["supp_id"] = _model.IBP_SuppID;
                dtrowHeader["bill_no"] = _model.bill_no;
                //string dateStr = _model.bill_date;
                //string[] parts = dateStr.Split('-');
                //string rearranged = $"{parts[2]}-{parts[1]}-{parts[0]}";
                //_model.bill_date = rearranged;
                ...
                dtrowHeader["address"] = _model.bill_add_id == null ? "0" : _model.bill_add_id;
                dtrowHeader["remarks"] = _model.remarks;
                dtrowHeader["ewb_no"] = _model.EWBNNumber;
                dtrowHeader["einv_no"] = _model.EInvoive;
                dtrowHeader["inter_br_id"] = _model.interbrnchid;
                dtrowHeader["oc_amt_with_tp"] = _model.DocSuppOtherCharges;
        ...
        private DataTable ToDtblAttachmentDetail(InterBranchPurchase_Model _model)
        {
            try
            {
        ...
        public ActionResult SaveIBPDetails(InterBranchPurchase_Model _model)
        {
            try
            {
                string SaveMessage = "";
                ...
                string Nurr = ""; string CN_Narr = "";
                if (_model.CancelFlag)
                {
                    Nurr = _model.Nurration + $" {Resource.Cancelled} {Resource.On} {DateTime.Now.ToString("dd-MM-yyyy hh:mm")}.";
                    CN_Narr = _model.CN_Nurration + $" {Resource.Cancelled} {Resource.On} {DateTime.Now.ToString("dd-MM-yyyy hh:mm")}.";
                }

                var _IBPInvoiceattch = TempData["ModelDataattch"] as DirectPurchaseInvoiceattch;
                TempData["ModelDataattch"] = null;
                string tds_amt = _model.TDS_Amount == null ? "0" : _model.TDS_Amount;
                SaveMessage = _IBP_ISERVICE.Insert_IBPDetails(DtblHDetail, DtblItemDetail, DtblSubItem, BatchItemTableData, SerialItemTableData,
            DtblTaxDetail, DtblIOCDetail, DtblOCTaxDetail, DtblOcTdsDetail, DtblVouDetail, DtblAttchDetail, CostCenterDetails, Nurr);
                ...
                if (SaveMessage == "DocModify")
                {
                    ...
                    return RedirectToAction("InterBranchPurchaseDetail");
                }
                else if (SaveMessage == "Cancelled")
                {
                    try
                    {
                        ...
                        var filePath = SavePdfDocToSendOnEmailAlert(_model.Inv_no, _model.Inv_dt, fileName, null);
                        _Common_IServices.SendAlertEmail(CompID, BrchID, "105101153", _model.Inv_no, "C", UserID, "", filePath);
                    }
                ...
                else
                {
                    ...
                    {
                        string Guid = "";
                        if (_IBPInvoiceattch != null)
                        {
                            if (_IBPInvoiceattch.Guid != null)
                            {
                                Guid = _IBPInvoiceattch.Guid;
                            }
                        }
                        string guid = Guid;
        ...
        public ActionResult ApproveIBPDetails(InterBranchPurchase_Model _model, string Inv_No, string Inv_Date
             ...
             , string docid, string WF_Status1, string Bp_Nurr, string Dn_Nurration)
        {
            try
            {
                ...
                string Result = _IBP_ISERVICE.ApproveDPIDetail(Inv_No, Inv_Date, docid, BrchID
        ...
        public string SavePdfDocToSendOnEmailAlert(string poNo, string poDate, string fileName, string PrintFormat)
        {
            DataTable dt = new DataTable();
            var commonCont = new CommonController(_Common_IServices);
            var data = GetPdfData("105101153", poNo, poDate);
            return commonCont.SaveAlertDocument(data, fileName);
        }
        public ActionResult GetSubItemDetails(string Item_id, string SubItemListwithPageData, string IsDisabled, string Flag, string Status, string Doc_no, string Doc_dt, string wh_id, string UomId)
        {
            try
            {
                GetCompDeatil();
                ...
                if (Status == "D" || Status == "F" || Status == "")
                {
                    ...
                    if (arr.Count > 0)
                    {
                        // Add columns dynamically based on first object's properties
                        foreach (JProperty prop in ((JObject)arr[0]).Properties())
                        {
                            dt.Columns.Add(prop.Name, typeof(string)); // or infer type if you want
                        }

                        // Loop through each JObject in the JArray and add rows
                ...
                {
                    dt = _IBP_ISERVICE.IBP_GetSubItemDetails(CompID, BrchID, Item_id, Doc_no, Doc_dt).Tables[0];
                }
        ...
        #region PDF
        public FileResult GenratePdfFile(InterBranchPurchase_Model _model)
        {
        ...
        public byte[] GetPdfData(string docId, string invNo, string invDt)
        {
            ...
            try
            {
                ...
                DataSet Details = _IBP_ISERVICE.GetIBPInvoiceDeatilsForPrint(CompID, BrchID, invNo, invDt);
                ViewBag.PageName = "PI";
                ...
                string htmlcontent = "";
                ViewBag.Title = "Inter Branch Purchase Invoice";
